...
def SADS(points, rgb, k):
    """
    Output the point indices sorted by information entropy in descending order.
    """
    dis, idx = knn_point_cloud(points, k)
    knn_points = points[idx]
    knn_rgb = rgb[idx]

    center = np.mean(knn_points, axis=1, keepdims=True)
    offsets = knn_points - center

    distances = np.linalg.norm(offsets, axis=-1)
    azimuth = np.arctan2(offsets[:, :, 1], offsets[:, :, 0])
    elevation = np.arctan2(offsets[:, :, 2], np.sqrt(offsets[:, :, 0]**2 + offsets[:, :, 1]**2))

    knn_data = np.concatenate([
        distances[..., None],
        azimuth[..., None],
        elevation[..., None],
        knn_points,
    ...
    fft_data = np.fft.fft(knn_data, axis=-1).astype(np.complex64)
    knn_fft = fft_data / np.linalg.norm(fft_data, axis=0, keepdims=True)

    x_min = np.min(knn_fft, axis=-1, keepdims=True)
    x_max = np.max(knn_fft, axis=-1, keepdims=True)
    knn_fft = (knn_fft - x_min) / ((x_max - x_min) + 1e-4)

    fft_overlap = np.sum(knn_fft, axis=-1, keepdims=False)
    fft_std = np.std(fft_overlap[:], axis=1)
    fft_std = np.real(fft_std) 
    exp_x = np.exp(fft_std - np.max(fft_std))
    softmax_score = exp_x / (np.sum(exp_x) + 1e-8)
    entropy = -softmax_score * np.log(softmax_score + 1e-9)
